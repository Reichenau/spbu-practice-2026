...
class DslExecutionWorkerActor(Actor[DslExecutionWorkerState, ExecutionWorkerMessage, ExecutionWorkerMessage]):
    ...
    def _evaluate_context_window(
        ...
        max_length = query.limit if isinstance(query.limit, int) else 1
        for size in range(1, min(len(candidates) + 1, max_length + 1)):
    ...
    def _within_constraints_satisfied(self, nodes: list[AstNode], query: Query) -> bool:
