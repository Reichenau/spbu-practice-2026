...
# -------------------- Единый класс запроса --------------------
@dataclass(slots=True)
class Query(Serializable):
    kind: str  # "FIND", "CONTEXT", "DISTANCE"
    source: Selector  # для FIND/CONTEXT — сущность; для DISTANCE — левый селектор
    target: Selector | None = None      # только для DISTANCE
    patterns: list[Pattern] | None = None  # только для CONTEXT
    within: list[WithinConstraint] = field(default_factory=list)
    where: Expression | None = None
    limit: int | PairLimit | None = None  # для CONTEXT — целое (длина окна), для DISTANCE — PairLimit, для FIND — игнорируется
    returns: list[ReturnItem] | None = None


# Для обратной совместимости можно определить алиасы (опционально)
FindQuery = Query
ContextQuery = Query
DistanceQuery = Query
DslQuery = Query
# Удаляем старые классы ContextQuery, FindQuery, DistanceQuery, но оставляем алиасы