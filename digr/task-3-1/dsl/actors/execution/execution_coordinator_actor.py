...
class DslExecutionCoordinatorActor(
    ...
    def on_idle_execute_dsl_query_request(self, message: ExecuteDslQueryRequest) -> DslExecutionCoordinatorState:
        ...
        self._current_query = message.query
        if message.query.kind == "FIND":
            return self._dispatch_find(message.query)
        if message.query.kind == "DISTANCE":
            return self._dispatch_distance(message.query)
        if message.query.kind == "CONTEXT":
            return self._dispatch_context(message.query)
        self._collector.tell(DslExecutionFailed(ValueError(f"Unknown query kind: {message.query.kind}")))
        return DslExecutionCoordinatorState.COMPLETED
    ...
    def on_evaluating_find_candidates_find_candidate_evaluated(
        ...
        if query is None or query.kind != "FIND":
    ...
    def on_evaluating_context_windows_context_window_evaluated(
        ...
        query = self._current_query
        if query is None or query.kind != "CONTEXT":
    ...
    def _dispatch_context(self, query: Query) -> DslExecutionCoordinatorState:
        ...
        max_window_length = query.limit if isinstance(query.limit, int) else None
        if max_window_length is None or max_window_length <= 0:
            max_window_length = 1
    ...
    def _dispatch_distance(self, query: Query) -> DslExecutionCoordinatorState:
        try:
            ...
            pairs = self._distance_calculator.calculate_pairs(
                ...
                query.target,
                query.within,
                query.limit if isinstance(query.limit, PairLimit) else PairLimit(mode="nearest"),
