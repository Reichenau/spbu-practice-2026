...
class DslTokenStreamParser:
    ...
    def _parse_context_query(self) -> Query:
        ...
        # Новый синтаксис: LIMIT <число>
        limit = self._parse_optional_limit()
        if span is None:
            if limit is None:
                raise self._error("CONTEXT query must specify window length (use LIMIT <number> or [<=N])")
            if not isinstance(limit, int):
                raise self._error("CONTEXT LIMIT must be a positive integer")
            # Создаём источник как Selector без предиката (только сущность)
            source = Selector(entity_name=entity_name)
        else:
            # Используем span, но нам нужен Selector для source, а ограничение будет в limit как int
            source = Selector(entity_name=entity_name)
            # Если был старый синтаксис, используем его ограничение как limit (int)
            # Преобразуем constraint.value в int, но сохраним только число, оператор игнорируем
            # (для CONTEXT всегда <=)
            if limit is None:
                limit = span.constraint.value
            # иначе limit уже задан, игнорируем span.constraint

        returns = self.parse_return_clause()
        self.consume_query_terminator()
        self.expect(TokenKind.EOF, "Unexpected tokens after CONTEXT query")
    ...
    def _parse_find_query(self) -> Query:
        ...
        where = self.parse_where_clause()
        within = self.parse_within_clauses()
        self._parse_optional_limit()  # игнорируем LIMIT для FIND
        returns = self.parse_return_clause()
        self.consume_query_terminator()
        self.expect(TokenKind.EOF, "Unexpected tokens after FIND query")
    ...
    # -------------------- Distance query --------------------
    def _parse_distance_query(self) -> Query:
        self.expect(TokenKind.DISTANCE, "Expected DISTANCE")
        left = self.parse_selector()
        self.expect(TokenKind.TO, "Expected TO after left selector")
        right = self.parse_selector()

        within = self.parse_within_clauses()
        limit_pairs = self._parse_limit_pairs()  # PairLimit
        returns = self.parse_return_clause()
        self.consume_query_terminator()
        self.expect(TokenKind.EOF, "Unexpected tokens after DISTANCE query")
    ...
    # -------------------- Новые методы для LIMIT --------------------
    def _parse_optional_limit(self) -> int | None:
        if self.match(TokenKind.LIMIT, TokenKind.LIMIT_PAIRS):
            token = self.expect(TokenKind.INTEGER, "Expected integer after LIMIT")
            return token.value
        return None

    def _parse_limit_pairs(self) -> PairLimit:
        if self.match(TokenKind.LIMIT, TokenKind.LIMIT_PAIRS):
            if self.current.kind is TokenKind.INTEGER:
                value = self.advance().value
                if value <= 0:
                    raise self._error("LIMIT integer must be positive")
                return PairLimit(mode="k", value=value)
            mode = self.parse_identifier("Expected LIMIT mode")
            if mode not in {"nearest", "all_nearest", "all"}:
                raise self._error("Expected LIMIT nearest, all_nearest, all, or positive integer")
