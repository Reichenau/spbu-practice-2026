...
## Архитектура DiGr целиком

DiGr состоит из трёх модулей с однонаправленными зависимостями:
`digr/engine`, `digr/relation-classifier`, `digr/ontology-pipeline`.

- **engine** — движок DSL (устройство ниже), используется обоими другими
  модулями.
- **relation-classifier** — классификатор типа связи между парой понятий
  (`TemplateRelationClassifier`). Для пары строится документ из куска текста,
  и для каждого из 9 типов связи через движок прогоняются `CONTEXT`-запросы
  по текстовым шаблонам; побеждает тип с наибольшим числом сработавших
  шаблонов.
- **ontology-pipeline** — сборка первичной онтологии курса
  (`build_primary_ontology.py`). Через движок находит понятия
  (`FIND definition` по `\odmkey`), строит кандидатные пары, классифицирует
  их через relation-classifier, сверяет с эталоном (`data/pairs_w_relation.json`),
  генерирует TDL-файлы по разделам для ONTOL (`build_tdl.py`) и грузит пары
  в Qdrant для Ontologer (`export_qdrant_chunks.py`).

Поток данных: текст лекций → понятия (engine) → кандидатные пары →
типизированные пары (relation-classifier) → сверка с эталоном → TDL (ONTOL)
+ Qdrant (Ontologer).

![Диаграмма деятельности пайплайна первичной онтологии](pipeline_activity_diagram.svg)

Ниже — устройство самого движка, на котором построены оба других модуля.

## Архитектура движка (engine)
